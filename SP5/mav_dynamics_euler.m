function [sys,x0,str,ts,simStateCompliance] = mav_dynamics_euler(t,x,u,flag,P)
...
function sys=mdlDerivatives(t,x,uu, P)
    ...
    pndot = (ct*cs)*u + (sp*st*cs - cp*ss)*v + (cp*st*cs + sp*ss)*w;
    pedot = (ct*ss)*u + (sp*st*ss + cp*cs)*v + (cp*st*ss - sp*cs)*w;
    pddot = -st*u + sp*ct*v + cp*ct*w;
